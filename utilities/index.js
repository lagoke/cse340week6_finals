...
/* ************************
 * Build the inventory order view HTML
 ************************** */
Util.buildOrderGrid = async function (req, res, next) {
  let data = await invModel.getOrderRecords()
  let grid = '<table id="customers"><tr><th>Vehicle Make</th><th>Vehicle Model</th><th>Price</th><th>Quantity</th></tr>'

  data.rows.forEach((row) => {
    grid += '<tr><td>' + row.inv_make + '</td><td>' + row.inv_model + '</td><td>$' + new Intl.NumberFormat('en-US').format(row.inv_price) + '</td><td>' + row.inv_quantity + '</tr>'
